	...
	<xsl:template match="/">
		<artifact>
			<xsl:attribute name="xsi:noNamespaceSchemaLocation">
				<xsl:value-of select="'C:/DOCUME~1/XPMUser/MYDOCU~1/CCFLIV~1/CCFLIV~1/samples/QCTFNG/QC2TF/xslt/TE8A5F~1.XSD'"/>
			</xsl:attribute>
			...
			<xsl:for-each select="$var1_instance/artifact">
				...
				<xsl:for-each select="BG_ACTUAL_FIX_TIME">
					<actualHours>
						<xsl:choose>
				...
				<xsl:for-each select="BG_ESTIMATED_FIX_TIME">
					<estimatedHours>
						<xsl:choose>
							<xsl:when test="string(('' = string(.))) != 'false'">
								<xsl:value-of select="'0'"/>
				...
				<xsl:for-each select="BG_RESPONSIBLE">
					<assignedTo>
						<xsl:value-of select="string(.)"/>
					</assignedTo>
				</xsl:for-each>
				<xsl:for-each select="BG_DESCRIPTION">
					<description>
						<xsl:value-of select="string(.)"/>
					</description>
				</xsl:for-each>
				<xsl:for-each select="BG_PRIORITY">
					<priority>
				...
				<xsl:for-each select="BG_STATUS">
					<status>
						<xsl:value-of select="string(.)"/>
					</status>
				</xsl:for-each>
				<xsl:for-each select="BG_SUMMARY">
					<title>
						<xsl:value-of select="string(.)"/>
					</title>
				</xsl:for-each>
				<xsl:for-each select="BG_DEV_COMMENTS">
					<CommentText>
						<xsl:value-of select="string(.)"/>
					</CommentText>
				</xsl:for-each>
