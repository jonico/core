	...
	<xsl:template match="/">
		<artifact>
			<xsl:attribute name="xsi:noNamespaceSchemaLocation">
				<xsl:value-of select="'C:/DOCUME~1/XPMUser/MYDOCU~1/CCFLIV~1/CCFLIV~1/samples/QCTFNG/TF2QC/xslt/QUALIT~1.XSD'"/>
			</xsl:attribute>
			...
			<xsl:for-each select="$var1_instance/artifact">
				...
				<xsl:for-each select="priority">
					<BG_PRIORITY>
						...
						<xsl:value-of select="string($var14_result_vmf1_inputtoresult)"/>
					</BG_PRIORITY>
				</xsl:for-each>
				<xsl:for-each select="assignedTo">
					<BG_RESPONSIBLE>
						<xsl:choose>
							<xsl:when test="string(('nobody' = string(.))) != 'false'">
								<xsl:value-of select="''"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="string(.)"/>
