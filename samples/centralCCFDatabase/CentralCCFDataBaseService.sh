...
# Application
APP_NAME="CentralCCFDataBase"
APP_LONG_NAME="CentralCCFDataBase"
...
# description: CentralCCFDataBase
...
### BEGIN INIT INFO
# Provides: CentralCCFDataBase
# Required-Start: $local_fs $network $syslog
# Should-Start: 
# Required-Stop:
# Default-Start: 2 3 4 5
# Default-Stop: 0 1 6
# Short-Description: CentralCCFDataBase
# Description: CentralCCFDataBase
### END INIT INFO
    ...
    if [ `id | sed 's/^uid=//;s/(.*$//'` != "0" ] ; then       
        ...
        if [ "$DIST_OS" = "solaris" ] ; then
            ...
            if [ -f /etc/init.d/$APP_NAME ] ; then
                ...
                for i in /etc/rc3.d/S99$APP_NAME /etc/rc3.d/K20$APP_NAME /etc/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
            ...
            if [ -f /etc/redhat-release -o -f /etc/redhat_version -o -f /etc/fedora-release ] ; then
                ...
                if [ -f /etc/init.d/$APP_NAME ] ; then
                    echo " Removing $APP_LONG_NAME daemon..."
                    for i in /etc/rc3.d/K20$APP_NAME /etc/rc5.d/K20$APP_NAME /etc/rc3.d/S99$APP_NAME /etc/init.d/$APP_NAME /etc/rc5.d/S99$APP_NAME
                    do
                        rm -f $i
                    done
                else
            ...
            if [ -f /sbin/init.d/$APP_NAME ] ; then
                echo " Removing $APP_LONG_NAME daemon..."
                for i in /sbin/rc3.d/K20$APP_NAME /sbin/rc3.d/S99$APP_NAME /sbin/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
            ...
            if [ -f /etc/rc.d/init.d/$APP_NAME ] ; then
                echo " Removing $APP_LONG_NAME daemon..."
                for i in /etc/rc.d/rc2.d/S99$APP_NAME /etc/rc.d/rc2.d/K20$APP_NAME /etc/rc.d/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
