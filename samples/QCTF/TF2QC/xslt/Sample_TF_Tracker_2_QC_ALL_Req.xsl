...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match='/ccf:artifact[@artifactType = "plainArtifact"]'>
		<artifact xmlns="http://ccf.open.collab.net/GenericArtifactV1.0">	
			...
			<xsl:variable name="requirementType" select='ccf:field[@fieldName="QC_REQUIREMENT_TYPE"]'/>			
			<xsl:choose>
	...
	<!-- begin of templates for non specific fields from TF to QC -->
	...
	<xsl:template match='ccf:field[@fieldName="QC_REQUIREMENT_TYPE"]' mode="nonSpecificFields">
		<field>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="fieldName">RQ_TYPE_ID</xsl:attribute>
			<xsl:value-of select="." />
		</field>
	</xsl:template>
	...
	<xsl:template match='ccf:field[@fieldName="description"]' mode="nonSpecificFields">
		<field>
			<xsl:copy-of select="@*" />
	...
	<xsl:template match='ccf:field[@fieldName="priority"]' mode="nonSpecificFields">
	...
	<xsl:template match='ccf:field[@fieldName="status"]' mode="nonSpecificFields">
	...
	<!-- end of templates for non specific fields from TF to QC -->	
	<!-- begin of templates for folder specific fields from TF to QC -->
	<xsl:template match='ccf:field[@fieldName="Reviewed"]' mode="folderSpecificFields">
		<field>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="fieldName">RQ_REQ_REVIEWED</xsl:attribute>
			<xsl:attribute name="fieldType">flexField</xsl:attribute>
			<xsl:value-of select="." />
		</field>
	</xsl:template>
	<!-- end of templates for folder specific fields from TF to QC -->
	<!-- begin of templates for functional specific fields from TF to QC -->
	<xsl:template match='ccf:field[@fieldName="Direct Cover Status"]' mode="functionalSpecificFields">
		<field>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="fieldName">RQ_REQ_STATUS</xsl:attribute>
			<xsl:attribute name="fieldType">flexField</xsl:attribute>
			<xsl:value-of select="." />
		</field>
	</xsl:template>
	<!-- end of templates for functional specific fields from TF to QC -->
