...
# Optimized for use with version 3.4.1 of the Wrapper.
...
### BEGIN INIT INFO
# Provides: JIRA2TF
# Required-Start: $local_fs $network $syslog CentralCCFDataBase
# Should-Start: 
# Required-Stop: CentralCCFDataBase
# Default-Start: 2 3 4 5
# Default-Stop: 0 1 6
# Short-Description: JIRA2TF
# Description: JIRA2TF
### END INIT INFO
    ...
    if [ `id | sed 's/^uid=//;s/(.*$//'` != "0" ] ; then       
        ...
        if [ "$DIST_OS" = "solaris" ] ; then
            ...
            if [ -f /etc/init.d/$APP_NAME ] ; then
                ...
                ln -s /etc/init.d/$APP_NAME /etc/rc3.d/K10$APP_NAME
                ln -s /etc/init.d/$APP_NAME /etc/rc3.d/S99$APP_NAME
            fi
            ...
            if [ -f /etc/redhat-release -o -f /etc/redhat_version -o -f /etc/fedora-release ]  ; then
                ...
                if [ -f /etc/init.d/$APP_NAME ] ; then
                    ...
                    ln -s /etc/init.d/$APP_NAME /etc/rc3.d/K10$APP_NAME 
                    ln -s /etc/init.d/$APP_NAME /etc/rc3.d/S99$APP_NAME
                    ln -s /etc/init.d/$APP_NAME /etc/rc5.d/S99$APP_NAME
                    ln -s /etc/init.d/$APP_NAME /etc/rc5.d/K10$APP_NAME
                fi
            fi
            ...
            if [ -f /sbin/init.d/$APP_NAME ] ; then
                ...
                ln -s /sbin/init.d/$APP_NAME /sbin/rc3.d/K10$APP_NAME
                ln -s /sbin/init.d/$APP_NAME /sbin/rc3.d/S99$APP_NAME
            fi
            ...
            if [ -f /etc/rc.d/init.d/$APP_NAME ] ; then
                ...
                ln -s /etc/rc.d/init.d/$APP_NAME /etc/rc.d/rc2.d/S99$APP_NAME
                ln -s /etc/rc.d/init.d/$APP_NAME /etc/rc.d/rc2.d/K10$APP_NAME
            fi
    ...
    if [ `id | sed 's/^uid=//;s/(.*$//'` != "0" ] ; then       
        ...
        if [ "$DIST_OS" = "solaris" ] ; then
            ...
            if [ -f /etc/init.d/$APP_NAME ] ; then
                ...
                for i in /etc/rc3.d/S99$APP_NAME /etc/rc3.d/K10$APP_NAME /etc/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
            ...
            if [ -f /etc/redhat-release -o -f /etc/redhat_version -o -f /etc/fedora-release ] ; then
                ...
                if [ -f /etc/init.d/$APP_NAME ] ; then
                    echo " Removing $APP_LONG_NAME daemon..."
                    for i in /etc/rc3.d/K10$APP_NAME /etc/rc5.d/K10$APP_NAME /etc/rc3.d/S99$APP_NAME /etc/init.d/$APP_NAME /etc/rc5.d/S99$APP_NAME
                    do
                        rm -f $i
                    done
                else
            ...
            if [ -f /sbin/init.d/$APP_NAME ] ; then
                echo " Removing $APP_LONG_NAME daemon..."
                for i in /sbin/rc3.d/K10$APP_NAME /sbin/rc3.d/S99$APP_NAME /sbin/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
            ...
            if [ -f /etc/rc.d/init.d/$APP_NAME ] ; then
                echo " Removing $APP_LONG_NAME daemon..."
                for i in /etc/rc.d/rc2.d/S99$APP_NAME /etc/rc.d/rc2.d/K10$APP_NAME /etc/rc.d/init.d/$APP_NAME
                do
                    rm -f $i
                done
            else
