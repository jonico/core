...
<xsl:stylesheet version="2.0"
	...
	<xsl:template
		...
		<field>
		    <xsl:attribute name="fieldName"><xsl:value-of select="$statusValue"></xsl:value-of>description</xsl:attribute>
	...
	<xsl:template
	 ...
	 <xsl:template
		...
		<field>
            ...
            <xsl:if test="$priorityValue = ''"><xsl:text>P5</xsl:text></xsl:if>
		</field>
	</xsl:template>
	<xsl:template
