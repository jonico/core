	...
	<xsl:template
		...
		<field>
		    ...
		    <xsl:attribute name="fieldType"><xsl:value-of select="@fieldType" /></xsl:attribute>
		  	...
		  	<xsl:if test="$statusValue = 'Open'"><xsl:text>Target Open</xsl:text></xsl:if>
			<xsl:if test="$statusValue = 'Closed'"><xsl:text>Target Closed</xsl:text></xsl:if>
			<xsl:if test="$statusValue = 'Pending'"><xsl:text>Target Fixed</xsl:text></xsl:if>
	  	</field>
	</xsl:template>
