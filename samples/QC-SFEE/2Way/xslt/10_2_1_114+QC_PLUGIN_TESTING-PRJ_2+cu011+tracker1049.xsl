	 ...
	 <xsl:template
		...
		<field>
		    ...
		    <xsl:attribute name="fieldType"><xsl:value-of select="@fieldType" /></xsl:attribute>
		  	...
		  	<xsl:attribute name="nullValueSupported"><xsl:value-of select="@nullValueSupported" /></xsl:attribute>
		  	<xsl:attribute name="alternativeFieldName"><xsl:value-of select="@alternativeFieldName" /></xsl:attribute>
		  	<xsl:if test="$typeValue = 'DefectCat'"><xsl:text>DefectCat</xsl:text></xsl:if>
			<xsl:if test="$typeValue = 'PatchCat'"><xsl:text>PatchCat</xsl:text></xsl:if>
			<xsl:if test="$typeValue = 'TaskCat'"><xsl:text>TaskCat</xsl:text></xsl:if>
			<xsl:if test="$typeValue = 'EnhancementCat'"><xsl:text>EnhancementCat</xsl:text></xsl:if>
			<xsl:if test="$typeValue = 'FeatureCat'"><xsl:text>FeatureCat</xsl:text></xsl:if> 
	  	</field>
	</xsl:template>
