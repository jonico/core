...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match='ccf:field[@fieldName="startDate"]'>
		<field>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="fieldName">startDate</xsl:attribute>
			<xsl:attribute name="fieldType">mandatoryField</xsl:attribute>
			<xsl:value-of select="." />
		</field>
	</xsl:template>
	<xsl:template match='ccf:field[@fieldName="releaseDate"]'>
		<field>
			<xsl:copy-of select="@*" />
			<xsl:attribute name="fieldName">endDate</xsl:attribute>
			<xsl:attribute name="fieldType">mandatoryField</xsl:attribute>
			<xsl:value-of select="." />
		</field>
	</xsl:template>
