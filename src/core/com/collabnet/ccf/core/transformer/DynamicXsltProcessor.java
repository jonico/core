...
public class DynamicXsltProcessor extends Component implements IDataProcessor {
	...
	public Object[] process(Object record) throws ProcessingException {
		...
		if (record instanceof Document) {
			...
			if (!StringUtils.isEmpty(this.xsltDir)) {
				...
				// make sure users did not tamper with immutable top level attributes
	...
	private static void restoreImmutableTopLevelAttributes(
		...
		try {
			...
			// special handling for artifact action that is only overridden if it is not set to ignore
			String newArtifactAction = XPathUtils.getAttributeValue(newRootElement, GenericArtifactHelper.ARTIFACT_ACTION, false);
	...
	private static void restoreAttribute(String attributeName,
		...
		newElement.addAttribute(attributeName,
				XPathUtils.getAttributeValue(originalElement, attributeName, false));
	}
