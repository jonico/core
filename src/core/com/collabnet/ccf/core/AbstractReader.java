...
public abstract class AbstractReader<T> extends Component implements
	...
	private static final String FIELD_MAPPING_PARAM = "//FIELD_MAPPING_PARAM | //field_mapping_param";
	...
	protected String getFieldMappingParam(Document syncInfo) {
		Node node = syncInfo.selectSingleNode(FIELD_MAPPING_PARAM);
		if (node == null)
			return null;
		return node.getText();
	}
	...
	public String getSourceRepositoryKind(Document syncInfo) {
		if (isCCF2xProcess()) {
			...
			try {
				...
				case CCF_CORE: {
					xsltDirectory = direction.name()
							+ File.separatorChar + FIELD_MAPPING_CORE_DIRECTORY
							+ File.separatorChar
							+ getFieldMappingParam(syncInfo)
							+ File.separatorChar;
					break;
				}
				case EXTERNAL_APP: {
