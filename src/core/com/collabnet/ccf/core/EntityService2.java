...
public class EntityService2 extends LifecycleComponent implements IDataProcessor {
	 ...
	 */
	private static final Log log = LogFactory.getLog(EntityService2.class);
		...
		try {
			...
			if (artifactType
				...
				if (StringUtils.isEmpty(targetParentArtifactId)) {
					...
					if (!isOnlyQuarantineAttachmentIfParentInHospital() || skipQuarantinedArtifact(null, sourceParentArtifactId, sourceSystemId, sourceParentRepositoryId, targetSystemId, targetParentRepositoryId, GenericArtifactHelper.ARTIFACT_TYPE_PLAIN_ARTIFACT, null, 0, true, repositoryMappingDirectionId)) {
						String cause = "Parent artifact "
							+ sourceParentArtifactId
							+ " for attachment "
							+ sourceArtifactId
	...
	private boolean skipQuarantinedArtifact(Element element,
			...
			boolean onlyCheckIfQuarantinedArtifactExists, String repositoryMappingDirectionId) {
