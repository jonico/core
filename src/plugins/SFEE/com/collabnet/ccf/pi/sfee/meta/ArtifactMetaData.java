package com.collabnet.ccf.pi.sfee.meta;
...
public class ArtifactMetaData {
	...
	public static FieldValueTypeValue getFieldValueType(String fieldName) {
		SFEEFields field = SFEEFields.valueOf(Character.toLowerCase(fieldName.charAt(0))+fieldName.substring(1));
		if(field != null){
	...
	public static FieldValueTypeValue getGAFieldValueType(String fieldType, String valueType){
		if(fieldType == TrackerFieldSoapDO.FIELD_TYPE_MULTISELECT || 
				fieldType == TrackerFieldSoapDO.FIELD_TYPE_MULTISELECT_USER){
			return FieldValueTypeValue.MULTI_SELECT_LIST;
		}
		else if(fieldType == TrackerFieldSoapDO.FIELD_TYPE_SINGLE_SELECT){
			return FieldValueTypeValue.LIST;
		}
		else if(fieldType == TrackerFieldSoapDO.FIELD_VALUE_TYPE_DATE ||
				fieldType == TrackerFieldSoapDO.FIELD_TYPE_DATE){
			return FieldValueTypeValue.DATETIME;
		}
		else if(fieldType == TrackerFieldSoapDO.FIELD_VALUE_TYPE_INTEGER){
			return FieldValueTypeValue.INTEGER;
		}
		else if(fieldType == TrackerFieldSoapDO.FIELD_VALUE_TYPE_USER){
			return FieldValueTypeValue.USER;
		}
		else if(fieldType == TrackerFieldSoapDO.FIELD_VALUE_TYPE_STRING){
			return FieldValueTypeValue.STRING;
		}
		else{
			return FieldValueTypeValue.STRING;
