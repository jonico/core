...
public class SFEEReader extends AbstractReader<Connection> {
	...
	public List<GenericArtifact> getArtifactAttachments(Document syncInfo,
		...
		try {
			attachments = attachmentHandler.listAttachments(connection
					...
					artifactIds, connection.getSfSoap(), this
		...
		try {
			...
			if (this.isIncludeFieldMetaData()) {
				trackerFields = trackerHandler.getFlexFields(connection
						.getSessionId(), sourceRepositoryId);
	...
	public List<ArtifactState> getChangedArtifacts(Document syncInfo) {
		...
		try {
			artifactRows = trackerHandler.getChangedTrackerItems(connection
					.getSessionId(), sourceRepositoryId, lastModifiedDate,
