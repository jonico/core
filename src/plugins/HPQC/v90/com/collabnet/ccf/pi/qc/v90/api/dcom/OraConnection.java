...
public class OraConnection {
	...
	public OraConnection() {
		...
		if ( url != null ) {
			try {
				oraProps.load( new FileInputStream( new File(url.getFile())));
					...
					log.fatal("Could not open oracle jdbc config file: " + url.getFile() , e);
			}
		} else {
			log.fatal("Could not find oracle jdbc config file: is oracle.jdbc.properties present and in classpath?");
		}
		...
		try {
			...
			log.fatal("Could not instantiate oracle driver " + driverClassName + ", is the file ojdbc6.jar present and in classpath?" , e);
		}
	...
	public IRecordSet executeSql ( String sql ) {
		...
		try {
			...
			log.error("Message:    " + e.getMessage());
			log.error("SQL State:  " + e.getSQLState());
			log.fatal("Failed SQL execution", e);
		} 
