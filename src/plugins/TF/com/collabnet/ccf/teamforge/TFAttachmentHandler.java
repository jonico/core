...
public class TFAttachmentHandler {
		...
		while (retryCall) {
			...
			try {
				...
				while (fileAttached) {
					try {
						fileAttached = false;
						connection.getTrackerClient().setArtifactData(soapDo, comment,
						...
						logConflictResolutor
								.warn(
										"Stale attachment update, trying again ...:",
										e);
						soapDo = connection.getTrackerClient().getArtifactData(artifactId);
						fileAttached = true;
					}
	 ...
	 * @throws RemoteException
	 */
	public byte[] getAttachmentData(Connection connection, String fileId, long size,
		...
		while (retryCall) {
			retryCall = false;
			DataHandler dataHandler = connection.getFileStorageClient().downloadFileDirect(folderId, fileId);
	...
	public void deleteAttachment(Connection connection, String attachmentId,
		...
		connection.getTeamForgeClient().deleteAttachment(artifactId, attachmentId);
		...
		if (artifact != null) {
			...
			att.setTargetArtifactLastModifiedDate(DateUtil
					.format(attachmentLastModifiedDate));
			att.setTargetArtifactVersion(Integer
					.toString(artifact.getVersion()));
		}
	}

	private AttachmentRow getAttachmentMetaData(Connection connection, String fileName,
			Date createdDate, String artifactId) throws RemoteException {
		AttachmentList attachmentsList = connection.getTeamForgeClient().listAttachments(artifactId);
		...
		for (String artifactId : artifactIds) {
			...
			for (AttachmentRow row : attachmentRows) {
				...
				if (createdDate.after(lastModifiedDate)) {
					...
					attachmentData = this.getAttachmentData(connection, row
