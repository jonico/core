...
public class TeamForgeTester {
	...
	public TeamForgeTester() throws FileNotFoundException, IOException {
		...
		userName = prop.getProperty(USER_NAME_PROPERTY); 
		...
		connection = Connection.getConnection(serverUrl, userName, password, null, Long.toString(System.currentTimeMillis()), null, false);
	...
	public void deleteAllPBIsInTF() throws RemoteException {
		ArtifactRow[] tfRows = connection.getTrackerClient().getArtifactList(pbiTracker, null).getDataRows();
	...
	public void deleteAllTasksInTF() throws RemoteException {
		ArtifactRow[] tfRows = connection.getTrackerClient().getArtifactList(taskTracker, null).getDataRows();
		for (ArtifactRow artifactRow : tfRows) {
			connection.getTrackerClient().deleteArtifact(artifactRow.getId());
		}
	}
	 ...
	 * @param release the name of the planning folder for the backlog item's release
	 * @param flexFields the custom fields
	 * @throws RemoteException if the TeamForge API can not be accessed
	 */
	public void createBacklogItem(final String title,
			final String description, 
			final String release,
			final FieldValues flexFields) throws RemoteException {connection.getTrackerClient().createArtifact(pbiTracker,
				title, description, null, null, STATUS_OPEN, null, 0, 0,
				0, false, null, null, getPlanningFolderId(release), flexFields, null, null, null);
	}
